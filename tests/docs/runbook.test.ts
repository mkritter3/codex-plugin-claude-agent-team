...
describe("Claude team session runbook", () => {
  it("documents the full public MCP workflow without chat-history dependence", async () => {
    ...
    expect(doc).toContain("cleanupBlocked");
	    ...
	    expect(doc).toContain("ollama launch claude --model <model> --yes -- <claude args>");
	    expect(doc).toContain("does not require `OLLAMA_API_KEY` when `ollama` is already signed in");
	    expect(doc).toContain("the only Ollama Claude Code mode that requires `OLLAMA_API_KEY`");
	    expect(doc).toContain("ollama-claude-code:glm-5.2");
	    expect(doc).toContain("ollama-claude-code:kimi-k2.7-code");
	    expect(doc).toContain("ANTHROPIC_BASE_URL");
	    expect(doc).toContain("scoped local Ollama env");
	    expect(doc).toContain("writeValidated");
	    expect(doc).toContain("Kimi K2.7 Code");
