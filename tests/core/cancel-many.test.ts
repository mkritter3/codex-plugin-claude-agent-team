...
import { cancelAgentRuns } from "../../src/core/cancel-many.js";
...
function controlResult(runId: string, status: AgentControlResult["status"]): AgentControlResult {
  return {
    ...
    message: `Cancel result for ${runId}.`
  };
}
...
describe("cancelAgentRuns", () => {
  it("cancels runs with bounded concurrency and preserves input order", async () => {
    const gates = Array.from({ length: 4 }, () => deferred<AgentControlResult>());
    const started: number[] = [];
    let active = 0;
    let maxActive = 0;

    const resultPromise = cancelAgentRuns(
      {
        concurrency: 2,
        runs: [
    ...
    gates[1]!.resolve(controlResult("run_1", "cancelled"));
    await flush();
    expect(started).toEqual([0, 1, 2]);

    gates[2]!.resolve(controlResult("run_2", "cancelling"));
    gates[0]!.resolve(controlResult("run_0", "cancelled"));
    await flush();
  ...
  it("records per-run failures and continues queued cancellations", async () => {
    const attempted: string[] = [];

    const result = await cancelAgentRuns(
      {
        concurrency: 1,
        runs: [
      ...
      },
      {
        async cancelRun(_cwd, runId) {
          ...
          return controlResult(runId, "cancelled");
        },
        async recoverStateCorruption() {
          throw new Error("should not recover state");
        }
  ...
  it("records recovery failures as per-run failures and continues queued cancellations", async () => {
    const attempted: string[] = [];
